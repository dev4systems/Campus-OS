    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="space-y-3">
        {dayData?.slots.map((slot, i) => (
          <div key={i} className="rounded-xl border border-border bg-card p-4 sm:p-5 hover:border-primary/30 transition-colors">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" /> {slot.time}</span>
                </div>
                <h3 className="font-semibold text-foreground">{slot.subject}</h3>
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><User className="h-3 w-3" /> {slot.professor}</span>
                  <span className="flex items-center gap-1"><Building2 className="h-3 w-3" /> {slot.building}</span>
