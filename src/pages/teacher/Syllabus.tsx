  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {deadlines.map((d) => (
          <div key={d.type} className={`rounded-xl border p-4 ${d.daysLeft <= 7 ? "border-status-danger/20 bg-status-danger/5" : "border-status-warning/20 bg-status-warning/5"}`}>
            <div className="flex items-center gap-2 mb-1">
              {d.daysLeft <= 7 ? <AlertCircle className="h-4 w-4 text-status-danger" /> : <Clock className="h-4 w-4 text-status-warning" />}
