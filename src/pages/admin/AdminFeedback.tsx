  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <div className="scroll-reveal rounded-xl border border-status-warning/20 bg-status-warning/5 p-4 text-center" style={{ transitionDelay: "70ms" }}>
          <p className="text-lg font-bold text-status-warning">{feedbackData.filter(f => f.status !== "resolved").length}</p>
          <p className="text-xs text-muted-foreground">Pending Review</p>
        </div>
        <div className="scroll-reveal rounded-xl border border-status-success/20 bg-status-success/5 p-4 text-center" style={{ transitionDelay: "140ms" }}>
          ...
          return (
            <div key={fb.id} className="scroll-reveal rounded-xl border border-border bg-card" style={{ transitionDelay: `${i * 70}ms` }}>
              <button onClick={() => setExpanded(expanded === fb.id ? null : fb.id)} className="w-full p-4 flex items-center justify-between text-left">
                ...
                {expanded === fb.id ? <ChevronUp className="h-5 w-5 text-muted-foreground shrink-0" /> : <ChevronDown className="h-5 w-5 text-muted-foreground shrink-0" />}
              </button>
              {expanded === fb.id && (
