  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-start justify-between">
        ...
        <Button size="sm"><Plus className="h-4 w-4 mr-1" /> Add Building</Button>
      </div>

      <div className="grid sm:grid-cols-2 gap-3">
        {campusBuildings.map((b) => (
          <div key={b.id} className="rounded-xl border border-border bg-card p-4 hover:border-primary/30 transition-colors">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground mt-1">
                  ...
                  <span className="flex items-center gap-0.5"><Layers className="h-3 w-3" /> {b.floors} floors</span>
                  <span>{b.rooms.length} rooms</span>
                  <span className="flex items-center gap-0.5"><MapPin className="h-3 w-3" /> {b.lat.toFixed(4)}, {b.lng.toFixed(4)}</span>
                </div>
